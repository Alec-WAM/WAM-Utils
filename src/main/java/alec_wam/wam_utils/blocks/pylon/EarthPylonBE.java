...
public class EarthPylonBE extends AbstractPylonBE {

	private int spawnDelay = 0;
	
	public EarthPylonBE(BlockPos p_155229_, BlockState p_155230_) {
		super(BlockInit.EARTH_PYLON_BE.get(), p_155229_, p_155230_);
	}
	...
	public void tickServer() {
		...
		//WAMUtilsMod.LOGGER.debug("Mobs: " + mobs.size());
		...
		if(mobs.size() < category.getMaxInstancesPerChunk()) {		
			...
			if(level.isLoaded(spawnBlockPos)) {
				if(level.isDay() && level.getRawBrightness(spawnBlockPos, 0) > 8) {
