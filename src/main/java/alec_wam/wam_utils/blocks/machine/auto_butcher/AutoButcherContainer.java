...
public class AutoButcherContainer extends WAMContainerMenu {

	public AutoButcherBE blockEntity;
    private Player playerEntity;
    private IItemHandler playerInventory;
	
	public AutoButcherContainer(int windowId, BlockPos pos, Inventory playerInventory, Player player) {
		super(ContainerInit.AUTO_BUTCHER_CONTAINER.get(), windowId);
		blockEntity = (AutoButcherBE) player.getCommandSenderWorld().getBlockEntity(pos);
        this.playerEntity = player;
        this.playerInventory = new InvWrapper(playerInventory);

        if (blockEntity != null) {
            addSlot(new SlotItemHandler(blockEntity.upgradeItems, 0, 71, 34) {
            	@Override
            	public Pair<ResourceLocation, ResourceLocation> getNoItemIcon() {
            		return Pair.of(InventoryMenu.BLOCK_ATLAS, GuiUtils.EMPTY_SLOT_BOOK);
            	}
            });
            addSlot(new SlotItemHandler(blockEntity.upgradeItems, 1, 89, 34));            
    ...
    public boolean stillValid(Player playerIn) {
        return stillValid(ContainerLevelAccess.create(blockEntity.getLevel(), blockEntity.getBlockPos()), playerEntity, BlockInit.AUTO_BUTCHER_BLOCK.get());
    }
