...
public class FlowerSimulatorContainer extends WAMContainerMenu {

	public FlowerSimulatorBE blockEntity;
    private Player playerEntity;
    private IItemHandler playerInventory;
	
	public FlowerSimulatorContainer(int windowId, BlockPos pos, Inventory playerInventory, Player player) {
		super(ContainerInit.FLOWER_SIMULATOR_CONTAINER.get(), windowId);
		blockEntity = (FlowerSimulatorBE) player.getCommandSenderWorld().getBlockEntity(pos);
        this.playerEntity = player;
        this.playerInventory = new InvWrapper(playerInventory);

        if (blockEntity != null) {
            addSlotRange(blockEntity.outputItems, 0, 44, 61, 5, 18);
    ...
    public boolean stillValid(Player playerIn) {
        return stillValid(ContainerLevelAccess.create(blockEntity.getLevel(), blockEntity.getBlockPos()), playerEntity, BlockInit.FLOWER_SIMULATOR_BLOCK.get());
    }
    ...
    public ItemStack quickMoveStack(Player playerIn, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            if(index < 5) {
            	//Chest Area
            	if(!this.moveItemStackTo(stack, 34, 41, false)) {
	            	if (!this.moveItemStackTo(stack, 5, 34, false)) {
            	...
            	if (!this.moveItemStackTo(stack, 5, 7, false)) {
	            	if (!this.moveItemStackTo(stack, 0, 5, false)) {
	            		if (index < 29 && !this.moveItemStackTo(stack, 34, 41, false)) {
	                      return ItemStack.EMPTY;
	            		}
	            		else if (!this.moveItemStackTo(stack, 5, 34, false)) {
	                        return ItemStack.EMPTY;
	              		}
	            	}
            	}
